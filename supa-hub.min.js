...
(function () {
    ...
    async function initSocket() {
        ...
        try {
            await loadSocketIO(); // WAITS for the library to exist

            const token = getModdioToken();
            if (!token) {
                console.error("Supa Hub: No login token found.");
                return;
            }

            // Using the specific handshake format modd.io requires
            ...
            socket.on("connect", () => {
                console.log("Supa Hub: Connected & Authenticated!");
                socket.emit("get dm users list");
            });
        ...
        users.forEach(user => {
            ...
            item.innerHTML = `
                ...
                <div class="dm-info">
                    ...
                    <div class="dm-status">${user.unreadMessagesCount > 0 ? user.unreadMessagesCount + ' new messages' : 'Online'}</div>
                </div>
            `;
            ...
            item.onclick = () => {
                ...
                if (chatNameHeader) chatNameHeader.innerText = user.username;
