...
(function () {
    ...
    function loadSocketIO() {
        return new Promise((resolve, reject) => {
            ...
            script.onload = () => {
                console.log("Supa Hub: Socket.io library loaded.");
                resolve();
            };
    ...
    function getModdioToken() {
        const match = document.cookie.match(new RegExp('(^| )jwt=([^;]+)'));
        return match ? match[2] : localStorage.getItem('guestUserToken');
    }
    ...
    async function initSocket() {
        ...
        try {
            await loadSocketIO();
            const token = getModdioToken();

            socket = io("https://chat.modd.io", {
                transports: ["websocket"],
                path: "/socket.io/",
                query: { EIO: 4 },
                auth: {
                    gameId: "64e8d6ca48404dbac0fc7fb2",
                    gameSlug: null,
                    token: token
                }
            });
            ...
            socket.on("connect", () => {
                console.log("Supa Hub: Connected!");
                socket.emit("get dm users");
            });

            socket.on("dm users list", (data) => {
                console.log("Supa Hub: Received user list", data);
                if (data && data.users) {
                    updateChatSidebar(data.users);
                }
            });
    ...
    const DEFAULT_LIB = [
        {
            ...
            children: [
                ...
                {
                    id: 'def_2', name: 'Full Commands System', type: 'script', icon: '⚡', json: {
                        "triggers": [],
                        "conditions": [
                        ...
                        "actions": [
                            { "type": "comment", "comment": "Command System Logic", "runMode": 0 }
                        ],
                        "name": "Full Chat Command System",
                        "parent": null,
                        "key": "generated_key_placeholder"
                    }
                }
            ]
        },
    ...
    const SUPA_CSS = `
        ...
        * { scrollbar-width: thin; scrollbar-color: var(--border) transparent; box-sizing: border-box; }
        ...
        /* Chat UI */
        .supa-chat-fab { position: fixed; bottom: 30px; right: 30px; width: 55px; height: 55px; background: var(--accent); border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 99999; box-shadow: 0 4px 20px rgba(0,0,0,0.6); transition: 0.2s; border: 2px solid rgba(255,255,255,0.1); }
        .supa-chat-fab:hover { transform: scale(1.1); box-shadow: 0 0 25px var(--accent); }
        .supa-chat-window { position: fixed; bottom: 100px; right: 30px; width: 650px; height: 450px; background: #0f0f12; border: 1px solid var(--border); border-radius: 16px; z-index: 99999; display: none; overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.8); backdrop-filter: blur(20px); }
        .supa-chat-window.visible { display: flex; }
        .chat-sidebar { width: 220px; background: rgba(10, 10, 14, 0.95); border-right: 1px solid var(--border); display: flex; flex-direction: column; }
        .chat-main { flex: 1; display: flex; flex-direction: column; background: var(--bg-deep); }
        .chat-header { padding: 15px 20px; border-bottom: 1px solid var(--border); font-weight: 700; font-size: 14px; display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.02); text-transform: uppercase; letter-spacing: 1px; }
        .dm-list { padding: 10px; overflow-y: auto; flex: 1; }
        .dm-item { display: flex; align-items: center; gap: 12px; padding: 10px; border-radius: 8px; cursor: pointer; transition: 0.2s; margin-bottom: 4px; border: 1px solid transparent; }
        .dm-item:hover { background: var(--glass); border-color: var(--border); }
        .dm-item.active { background: rgba(99, 102, 241, 0.1); border-color: var(--accent); }
        .dm-avatar { width: 34px; height: 34px; border-radius: 8px; background: #333; object-fit: cover; }
        .dm-info { display: flex; flex-direction: column; overflow: hidden; }
        .dm-name { font-size: 13px; font-weight: 600; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .dm-status { font-size: 10px; color: var(--text-dim); }
        .chat-messages { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 12px; }
        .msg-row { display: flex; gap: 10px; }
        .msg-bubble { background: var(--glass); padding: 8px 14px; border-radius: 0 12px 12px 12px; border: 1px solid var(--border); max-width: 80%; font-size: 13px; line-height: 1.5; }
        .msg-own .msg-bubble { background: var(--accent); color: white; border-radius: 12px 0 12px 12px; margin-left: auto; border: none; }
        .msg-own { flex-direction: row-reverse; }
        .chat-input-area { padding: 15px; border-top: 1px solid var(--border); display: flex; gap: 12px; background: rgba(255,255,255,0.02); align-items: center; }
        .chat-input { flex: 1; background: #000; border: 1px solid var(--border); border-radius: 10px; color: white; padding: 10px 15px; font-size: 13px; outline: none; transition: 0.2s; }
        .chat-input:focus { border-color: var(--accent); }
        .chat-send { background: var(--accent); color: white; border: none; width: 36px; height: 36px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 16px; }
    ...
    function renderLibList(filter = '') {
        ...
        const renderItems = (items, container, isUserFolder = false) => {
            items.forEach(item => {
                if (filter && item.type === 'script') return;
    ...
    function injectLibrary() {
        ...
        if (!target || target.parentElement.querySelector('.supa-script-lib')) {
            target = document.querySelector('winbox entities-window focus no-full max');
            return;
        }
    ...
    function updateChatSidebar(users) {
        ...
        users.forEach(user => {
            ...
            const avatar = user.profilePicture || "https://cache.modd.io/profile/default_profile_picture_human.png";

            userDiv.innerHTML = `
                <div class="flex flex-row items-center cursor-pointer rounded-md px-1 py-1 gap-2 w-full h-8 hover:bg-gray-800 transition-colors">
                    <div class="relative">
                        <img src="${avatar}" class="rounded w-[30px] h-[30px] object-contain">
                    </div>
                    <div class="flex gap-0 text-ellipsis truncate text-sm font-medium w-full items-center justify-between">
                        <div class="w-full truncate">${user.username}</div>
                        ${user.unreadMessagesCount > 0 ? `<div class="bg-red-500 rounded-full h-4 w-4 text-[10px] flex items-center justify-center">${user.unreadMessagesCount}</div>` : ''}
                    </div>
                </div>
            `;
            ...
            userDiv.onclick = () => {
                ...
                socket.emit('join room', { roomId: user.roomId });
                const header = document.querySelector('.text-sm.lg\\:text-base');
                if (header) header.innerText = user.username;
            };
    ...
    function buildChat() {
        if (document.getElementById('supa-chat-root')) return;

        initSocket();

        const root = document.createElement('div');
        root.id = 'supa-chat-root';
        root.innerHTML = `
        <div class="supa-chat-fab">💬</div>
        <div class="supa-chat-window">
            <div class="chat-sidebar">
                <div class="chat-header">Conversations</div>
                <div class="dm-list" id="dm-list-container">
                    <div style="padding:20px; opacity:0.5; font-size:12px;">Loading chats...</div>
                </div>
            </div>
            <div class="chat-main">
    ...
    `;
        ...
        const fab = root.querySelector('.supa-chat-fab');
        const win = root.querySelector('.supa-chat-window');

        fab.onclick = () => {
            win.classList.toggle('visible');
            isChatOpen = win.classList.contains('visible');
        };

        root.querySelector('#close-chat').onclick = () => {
        ...
        app.innerHTML = `
            <div class="supa-sidebar">
                ...
                <div class="sidebar-footer">
                    ...
                    <div style="opacity:0.6;">Modd.io UI ReImagined :)</div><br>
                    <div style="opacity:0.6;">Beta V1.341 TEST</div>
                </div>
            </div>
    ...
    async function render(viewOverride = null) {
        ...
        try {
            if (view === 'profile') {
                ...
                let useridToNum = Number(uJson.data._id.replace(/[a-zA-Z]/g, ''));
                let userCreatedAt = formatIsoTime(uJson.data.createdAt);
                ...
                container.innerHTML = `
                    <div style="background:var(--glass); border:1px solid var(--border); border-radius:24px; padding:40px; display:flex; align-items:center; gap:35px; margin-bottom:40px;">
                        <img src="${(uJson.data.local.username === 'm0dE') ? 'https://cdn.discordapp.com/avatars/221728407615045642/1e9dd14758d26ddc02c130aa284b7210.webp' : (uJson.data.local.profilePicture || 'https://cache.modd.io/profile/default_profile_picture_human.png')}" style="width:120px; height:120px; border-radius:20px; border:2px solid var(--accent); object-fit:cover;">
                        <div>
                            ...
                            <div style="display:flex; gap:30px;">
                                ...
                                <div style="position: absolute; top: 3vw; left: 10vw; font-size:13px; color:var(--text-dim); text-transform:uppercase;">Created At ${userCreatedAt}</div></div>
                            </div>
                        </div>
                    </div>
                `;
        ...
        games.forEach(g => {
            ...
            card.innerHTML = `
                <div class="card-img-wrap">
                    ...
                    ${g.activeCreators > 0 ? '<div class="creator-online" title="Creator is in-game!"><div class=""><div class="flex items-center justify-center rounded bg-green-500 px-1.5 py-0.5 mr-1.5 text-white" style="position: absolute;z-index: 10000000;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="1 0 20 20" fill="currentColor" aria-hidden="true" class="h-3 w-3"><path fill-rule="evenodd" d="M14.5 10a4.5 4.5 0 004.284-5.882c-.105-.324-.51-.391-.752-.15L15.34 6.66a.454.454 0 01-.493.11 3.01 3.01 0 01-1.618-1.616.455.455 0 01.11-.494l2.694-2.692c.24-.241.174-.647-.15-.752a4.5 4.5 0 00-5.873 4.575c.055.873-.128 1.808-.8 2.368l-7.23 6.024a2.724 2.724 0 103.837 3.837l6.024-7.23c.56-.672 1.495-.855 2.368-.8.096.007.193.01.291.01zM5 16a1 1 0 11-2 0 1 1 0 012 0z" clip-rule="evenodd"></path><path d="M14.5 11.5c.173 0 .345-.007.514-.022l3.754 3.754a2.5 2.5 0 01-3.536 3.536l-4.41-4.41 2.172-2.607c.052-.063.147-.138.342-.196.202-.06.469-.087.777-.067.128.008.257.012.387.012zM6 4.586l2.33 2.33a.452.452 0 01-.08.09L6.8 8.214 4.586 6H3.309a.5.5 0 01-.447-.276l-1.7-3.402a.5.5 0 01.093-.577l.49-.49a.5.5 0 01.577-.094l3.402 1.7A.5.5 0 016 3.31v1.277z"></path></svg></div></div></div>' : ''}
    ...
    syncLocalUser().then(() => {
        setInterval(() => {
            ...
            if (location.pathname.startsWith('/play/')) {
                const chatRoot = document.getElementById('supa-chat-root');
                ...
                buildChat();
                const chatRoot = document.getElementById('supa-chat-root');
                if (chatRoot) chatRoot.style.display = 'block';
            }
        }, 300);
    });
})();
