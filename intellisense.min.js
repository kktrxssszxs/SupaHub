/* ==========================================================================
   SUPA HUB LIBRARY & PARSER MODULE - V5 (Execution Fix)
   Features:
   - Fixed "Not Defined" errors by improving Proxy execution context
   - Auto-Sanitizer for reserved keywords
   - Fixed Intellisense/Autocomplete crash
   ========================================================================== */
...
(function(global) {
    ...
    function compile(jsCode, meta) {
        ...
        // SANITIZATION: Wrap reserved keywords used as keys in quotes
        const sanitizedCode = jsCode.replace(/([{,]\s*)(if|else|then|return|break|while|for|switch|case|default|function|var|let|const|try|catch|finally|throw|new|typeof|instanceof|void|delete|in|do|with|debugger|class|enum|extends|export|import|super)\s*:/g, '$1"$2":');
        ...
        // Populate sandbox with Modd Actions
        MODD_ACTIONS.forEach(act => {
            sandbox[act] = (args) => {
                const action = createAction(act, args);
                __ACTIONS__.push(action);
                return action;
            };
        });

        // Add special "condition" helper
        sandbox.condition = (args) => {
            const action = createAction('if', args);
            __ACTIONS__.push(action);
            return action;
        };

        // Populate sandbox with Modd Params
        MODD_PARAMS.forEach(param => {
            sandbox[param] = (args) => ({ function: param, ...(args || {}) });
        });

        // Handle property access that isn't pre-defined (like window/console)
        const proxy = new Proxy(sandbox, {
            get: (target, prop) => {
                if (prop in target) return target[prop];
                return global[prop];
            },
            has: (target, prop) => true // Force 'with' to look in proxy
        });

        try {
            // Updated runner: Ensure proxy is explicitly bound to 'this' and scope
            const runner = new Function('sandbox', `with(sandbox) { (function() { ${sanitizedCode} }).call(sandbox); }`);
            runner(proxy);
