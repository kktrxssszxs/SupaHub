/* ==========================================================================
   ...
   Features:
   - Functional Compiler (No more "Parsing complex" errors)
   - Improved Decompiler (thisEntity() vs action({}))
   - Monaco Intellisense with Documentation Hovers
   ========================================================================== */
...
(function(global) {
    ...
    // --- COMPILER (FUNCTIONAL COLLECTOR) ---
    function compile(jsCode, meta) {
        const paramFuncs = MODD_PARAMS.map(p => `const ${p} = (args) => ({ function: "${p}", ...(args || {}) });`).join('\n');
        const actionFuncs = MODD_ACTIONS.map(a => `const ${a} = (args) => { const obj = { type: "${a}", ...(args || {}) }; __ACTIONS__.push(obj); return obj; };`).join('\n');
            ...
            if (window.unsafeWindow?.monaco) {
                    ...
                    provideCompletionItems: (model, position) => {
                        ...
                        const suggestions = [
                            ...MODD_ACTIONS.map(a => ({
                                ...
                                documentation: { value: `**Action**: ${a}\n\n[Modd.io Documentation](https://github.com/moddio/moddio-docs/tree/main/scripting-reference)` }
                            })),
                            ...MODD_PARAMS.map(p => {
                                const isGetter = ["thisEntity", "allPlayers", "lastTriggeringUnit", "mouseCursorPosition"].includes(p);
                                return {
                                    label: p, kind: 6, insertText: isGetter ? p + '()' : p + '({  })', detail: 'Param', range,
                ...
                document.getElementById('supa-compile-btn').onclick = () => {
                    try {
                        ...
                        if (mainEditor) {
                            // Switch to JSON view briefly to set the value
