/* ==========================================================================
   ...
   Features:
   - Fixed "Unexpected token if" by treating 'if' as a Modd Action
   - Functional Compiler with better scope handling
   - Improved Decompiler
   ========================================================================== */
...
(function(global) {
    ...
    function compile(jsCode, meta) {
        // Define all Modd.io Params as functions returning JSON
        const paramFuncs = MODD_PARAMS.map(p => `const ${p} = (args) => ({ function: "${p}", ...(args || {}) });`).join('\n');
        // Define all Modd.io Actions as functions that push to a global array
        // We use 'condition' instead of 'if' to avoid JS keyword conflicts
        const actionFuncs = MODD_ACTIONS.map(a => {
            if (a === 'if') return `const condition = (args) => { const obj = { type: "if", conditions: args.if || [], actions: args.then || [], elseActions: args.else || [] }; __ACTIONS__.push(obj); return obj; };`;
