/* ==========================================================================
   SUPA HUB LIBRARY & PARSER MODULE - V7 (Reserved Token Fix)
   Features:
   - Reserved Keyword Mapping: Maps 'if', 'else', etc., to safe internal tokens
   - Explicit Scope Injection: Defines all actions/params as local variables
   - Fixed "Unexpected token 'if'" and "is not defined" errors
   ========================================================================== */
...
(function(global) {
    ...
    const JS_RESERVED = ["if", "else", "then", "return", "break", "while", "for", "switch", "case", "default", "function", "var", "let", "const", "try", "catch", "finally", "throw", "new", "typeof", "instanceof", "void", "delete", "in", "do", "with", "debugger", "class", "enum", "extends", "export", "import", "super"];

    function decompile(json) {
        if (!json) return "// New Script\n\n";
        let code = `// Script: ${json.name || 'Untitled'}\n\n`;

        function escapeStringForCode(s) {
        ...
        function parseVal(v) {
            ...
            if (typeof v === 'string') return `"${escapeStringForCode(v)}"`;
        ...
        function parseAction(act, indent = "") {
            if (!act) return "";
            if (act.type === 'comment') return `${indent}comment({ "comment": "${escapeStringForCode(act.comment || '')}" });`;
            if (act.type === 'if') {
    ...
    function compile(jsCode, meta) {
        ...
        // sanitizedCode holds the user's JS text. Do NOT attempt to quote keys with a regex here.
        let sanitizedCode = (typeof jsCode === 'string') ? jsCode : '';

        // Map reserved keywords to safe internal tokens, but only when they're used as function calls
        // (lookahead ensures '(' follows). The negative lookbehind avoids touching quoted keys or property access.
        JS_RESERVED.forEach(word => {
            // Use try/catch around RegExp construction in case environment doesn't support lookbehind
            try {
                const regex = new RegExp(`(?<!["'\\.])\\b${word}\\b(?=\\s*\\()`, 'g');
                sanitizedCode = sanitizedCode.replace(regex, `__${word}__`);
            } catch (err) {
                // Fallback (less precise) if lookbehind unsupported: only match word at start or after whitespace
                const fallback = new RegExp(`(^|\\s)\\b${word}\\b(?=\\s*\\()`, 'g');
                sanitizedCode = sanitizedCode.replace(fallback, `$1__${word}__`);
            }
        });
        ...
        const createAction = (type, args) => {
            ...
            if (type === 'if') {
                return {
                    type: "if",
                    conditions: aargs.if || aargs["if"] || [],
                    actions: aargs.then || [],
                    elseActions: aargs.else || []
                };
            }
            return { type, ...(aargs || {}) };
        };

        // Build scope definitions safely
        let definitions = "";
        MODD_ACTIONS.forEach(act => {
            // create a safe variable name for the action (prefix reserved words)
            const varName = JS_RESERVED.includes(act) ? `__${act}__` : act;
            // define function that records the action
            definitions += `const ${varName} = (args) => { const a = createAction("${act}", args); __ACTIONS__.push(a); return a; };\n`;
        });
        MODD_PARAMS.forEach(param => {
            const varName = JS_RESERVED.includes(param) ? `__${param}__` : param;
            definitions += `const ${varName} = (args) => ({ function: "${param}", ...(args || {}) });\n`;
        });
        // give a user-friendly alias for if-condition building
        definitions += `const condition = (args) => { const a = createAction("if", args); __ACTIONS__.push(a); return a; };\n`;

        try {
            // Create a wrapper function. Pass sanitized code as an argument to avoid injecting it directly
            // into the wrapper string (prevents breaking the wrapper if code contains quotes/backticks).
            const wrapper = new Function('createAction', '__ACTIONS__', '__CODE__', `
                "use strict";
                ${definitions}
                try {
                    // Evaluate user code within this scope so the action/param definitions are visible.
                    eval(__CODE__);
                } catch (err) {
                    throw err;
                }
            ...
            // Execute wrapper with createAction and actions array and the sanitized code string.
            wrapper(createAction, __ACTIONS__, sanitizedCode);
            ...
            modal.innerHTML = `
                <div style="background:#0a0a0e; border:1px solid #333; border-radius:15px; width:100%; height:100%; display:flex; flex-direction:column; overflow:hidden;">
                    <div style="padding:15px; border-bottom:1px solid #333; display:flex; justify-content:space-between; color:white; font-weight:bold;">
                        ...
                        <button id="close-supa-editor" style="background:none; border:none; color:white; cursor:pointer;">✕</button>
                    </div>
                ...
                document.getElementById('supa-compile-btn').onclick = () => {
                    try {
                        ...
                        if (mainEditor) {
                            ...
                            setTimeout(() => {
                                if (mainEditor) mainEditor.setValue(JSON.stringify(result, null, 4));
                                modal.remove();
                                disposable.dispose();
                            }, 100);
                        }
