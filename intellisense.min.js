/* ==========================================================================
   SUPA HUB LIBRARY & PARSER MODULE - V3 (Proxy Sandbox)
   Features:
   - Proxy-based sandbox to handle reserved keywords (if, else, return, etc.)
   - Automatically handles quoted and unquoted keys
   - Improved decompiler for cleaner code output
   ========================================================================== */
...
(function(global) {
    ...
    function compile(jsCode, meta) {
        // We create a global actions array
        const __ACTIONS__ = [];
        ...
        // The Proxy interceptor for the Sandbox
        const sandboxHandler = {
            get: (target, prop) => {
                // Return our special condition function for 'condition' call
                if (prop === 'condition') {
                    return (args) => {
                        const act = createAction('if', args);
                        __ACTIONS__.push(act);
                        return act;
                    };
                }
                // Check if it's a Modd Action
                if (MODD_ACTIONS.includes(prop)) {
                    return (args) => {
                        const act = createAction(prop, args);
                        __ACTIONS__.push(act);
                        return act;
                    };
                }
                // Check if it's a Modd Param/Getter
                if (MODD_PARAMS.includes(prop)) {
                    return (args) => ({ function: prop, ...(args || {}) });
                }
                // Fallback to global/native
                return target[prop] || global[prop];
            }
        };

        const sandboxProxy = new Proxy({}, sandboxHandler);

        try {
            // Using a strictly scoped function with the proxy
            const runner = new Function('proxy', `with(proxy) { ${jsCode} }`);
            runner(sandboxProxy);
            ...
            // Error mapping for common JS keywords
            if (e.message.includes("Unexpected token 'else'") || e.message.includes("Unexpected token 'if'")) {
                 throw new Error("Syntax Error: You used a reserved keyword (if/else) unquoted. The decompiler should fix this, but make sure your object keys are strings like {\"if\": ...}");
            }
