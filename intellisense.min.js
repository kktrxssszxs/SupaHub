...
(function(global) {
    ...
    function compile(jsCode, meta) {
        ...
        // 1. Wrap keys in quotes
        let sanitizedCode = jsCode.replace(/([{,]\s*)([a-zA-Z_0-9]+)\s*:/g, '$1"$2":');
        
        // 2. Map reserved keywords to internal functions (e.g. if() -> __if__())
        ...
        const createAction = (type, args) => {
            if (type === 'if') {
                return { type: "if", conditions: args.if || args["if"] || [], actions: args.then || [], elseActions: args.else || [] };
            }
            return { type, ...(args || {}) };
        };
        ...
        try {
            const wrapper = new Function('createAction', '__ACTIONS__', `
                ${definitions}
                try {
                    ${sanitizedCode}
                } catch (err) {
            ...
            wrapper(createAction, __ACTIONS__);
